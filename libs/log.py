...
# 路径设置
relative_directory = pathlib.Path(__file__).parent.parent  # FNT-SAN代码相对路径
result_save_dir = relative_directory.joinpath('results')  # 结果保存目录
log_path = result_save_dir.joinpath('FntScan.log')  # FNT-SAN日志保存路径
...
# 命令终端日志级别默认为INFOR
logger.add(sys.stderr, level='TRACE', format=stdout_fmt, enqueue=True)
# 日志文件默认为级别为DEBUG
logger.add(log_path, level='ERROR', format=logfile_fmt, enqueue=True, encoding='utf-8')
